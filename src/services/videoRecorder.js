...
export default class CanvasRecorder {
  constructor(canvas, { mimeType = chooseSupportedMimeType(), fps = 30, bitsPerSecond } = {}) {
    if (!canvas || !canvas.captureStream) {
      throw new Error("当前环境不支持 canvas.captureStream()");
    }
    if (!mimeType) {
      throw new Error("当前浏览器不支持可用的视频编码（MediaRecorder MIME type）。");
    }
    this.canvas = canvas;
  ...
  stop() {
    if (!this.isRecording()) return Promise.resolve(this._blob);
